...
void parallel_gaussianElimination(int N){
 ...
 void serial_gaussianElimination(int N){
	for (int pivot=0; pivot<N; pivot++){
        for (int row=pivot+1; row<N; row++){
            double f = matcopy[row][pivot]/matcopy[pivot][pivot];
            for (int col=pivot+1; col<=N; col++){
                matcopy[row][col] -= matcopy[pivot][col]*f;
            }
			matcopy[row][pivot] = 0;
        }
    }
    for (int row = N-1; row >= 0; row--){
        X[row] = matcopy[row][N];
        for (int col=row+1; col<N; col++){
            X[row] -= matcopy[row][col]*X[col];
        }
        X[row] = X[row]/matcopy[row][row];
    }
}
int main(){
	int N=700;
	...
	gettimeofday(&TimeValue_Start, &TimeZone_Start);
	parallel_gaussianElimination(N);
	gettimeofday(&TimeValue_Final, &TimeZone_Final);
	time_start = TimeValue_Start.tv_sec * 1000000 + TimeValue_Start.tv_usec;
	time_end = TimeValue_Final.tv_sec * 1000000 + TimeValue_Final.tv_usec;
	time_overhead = (time_end - time_start)/1000000.0;
